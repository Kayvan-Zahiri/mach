...
def beamform_fp16(
    ...
    ``channel_data`` is the complex64 data as interleaved float16 (re, im) pairs
    viewed as uint16, shape ``(n_rx, n_samples, 2 * n_frames)``:
    ``iq.view(cp.float32).astype(cp.float16).view(cp.uint16)``. Compute and
    ``out`` stay float32 / complex64.
    """
